...
-- Force undo point after MIDI item change
function Force_MIDI_Undo_Point()
...
local item = r.GetMediaItemTake_Item(r.MIDIEditor_GetTake(r.MIDIEditor_GetActive()))
...
local start_time, end_time = r.GetSet_ArrangeView2(0, false, 0, 0, start_time, end_time) -- isSet false
...
function Get_TCP_Under_Mouse() -- based on the above function Get_Object_Under_Mouse_Curs()
-- r.GetTrackFromPoint() covers the entire track timeline hence isn't suitable for getting the TCP
local curs_pos = r.GetCursorPosition() -- store current edit curs pos
local start_time, end_time = r.GetSet_ArrangeView2(0, false, 0, 0, start_time, end_time) -- isSet false, screen_x_start, screen_x_end are 0 to get full arrange view coordinates // get time of the current Arrange scroll position to use to move the edit cursor away from the mouse cursor
r.PreventUIRefresh(1)
...
function Insert_Empty_Item_To_Display_Text(output) -- relies on Re_Store_Selected_Objects(); for the item notes to recognize line breaks in the output they must be replaced with '\r\n' if the string wasn't previously formatted in the notes field https://forum.cockos.com/showthread.php?t=214861#2
...
function Get_Arrange_and_Header_Heights1() -- fetches data from a temporary project tab, WORKS, but isn't ideal because of being way too intrusive // if no SWS estension only works if the program window is fully open

-- !!!!!!!!!!! since build 6.76 there's a new preference for maximum vertical zoom which will break the function if not 100% as if affects the action 'View: Toggle track zoom to maximum height' use here to get Arrange height; it will still be possible to calculate it using the action, e.g. (track_height/max_height)*100 to find 100% value 
...
local wnd_h_offset = sws and top or 0 -- to add when calculating absolute track top edge coordinate inside Get_Item_Track_Segment_At_Mouse() function when the sws extension is installed to be able to get segments in the shrunk program window, in this case 'top' value represents difference between program window top and full screen top coordinates which is needed to match mouse cursor Y coordinate which is absolute i.e. is relative to the full screen size
	...
	local tr_height = r.GetMediaTrackInfo_Value(tr, 'I_TCPH') -- not including envelopes, action 40113 above doesn't take envs into account // equals Arrange height
...
function Get_Arrange_and_Header_Heights2() -- if no SWS extension only works if the program window is fully open
...
local track, info = r.GetTrackFromPoint(x, y)
	...
	local tr_height = r.GetMediaTrackInfo_Value(ref_tr, 'I_TCPH') -- not including envelopes, action 40113 doesn't take envs into account 
	r.Main_OnCommand(40110, 0) -- View: Toggle track zoom to minimum height
...
function GetPlayPosition3() -- using GetPlayPosition2() only when transport runs because the function runs independently of it 
-- https://forum.cockos.com/showthread.php?p=2617467
-- https://forum.cockos.com/showthread.php?t=273086
-- GetPlayPosition2() initial time depends on the edit cursor position, so once transport starts the value changes to the one corresponding to the current edit cursor position
local pos
...
or {r.my_getViewport(0, 0, 0, 0, 0, 0, 0, 0, true)} -- true - work area, false - the entire screen // https://forum.cockos.com/showthread.php?t=195629#4
	if #dimens_t == 5 then table.remove(dimens_t, 1) end -- remove retval value if BR's function
local rt, top, lt, bot = table.unpack(dimens_t)
local start_time, end_time = r.GetSet_ArrangeView2(0, false, 0, 0, start_time, end_time) -- isSet false, screen_x_start & screen_x_end both 0 = GET // https://forum.cockos.com/showthread.php?t=227524#2 the function has 6 arguments
...
reaper.CSurf_TrackToID(tr, true) == 0 -- true is MCP view
